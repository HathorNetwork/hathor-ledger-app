...
class CreateTokenTransaction:
    ...
    def from_bytes(cls, hexa: Union[bytes, BytesIO]):
        ...
        if tx_version != CREATE_TOKEN_TX_VERSION:
            raise ValueError(f"CreateTokenTransaction with version {tx_version} not supported")
    ...
    def __str__(self):
        ...
        return f"CreateTokenTransaction(symbol={self.symbol}, name={self.name}, inputs={sinputs}, outputs={soutputs})"
