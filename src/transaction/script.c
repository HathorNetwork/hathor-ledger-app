...
bool identify_p2pkh_script(buffer_t *in, size_t script_len) {
    ...
    if ((in == NULL) || ((script_len != 25) && (script_len != 31)) || (in->size - in->offset < script_len)) {
...
bool identify_p2sh_script(buffer_t *in, size_t script_len) {
    ...
    if ((in == NULL) || ((script_len != 23) && (script_len != 29)) || (in->size - in->offset < script_len)) {
        return false;
    }
...
bool identify_data_script(buffer_t *in, size_t script_len) {
    ...
    int expected_len = 2;
    int data_len = 0;
    if ((in == NULL) || (script_len < expected_len) || (script_len > MAX_DATA_SCRIPT_LEN) || (in->size - in->offset < script_len)) {
        return false;
    }
...
uint8_t read_data_script(buffer_t *in, size_t script_len, uint8_t out[MAX_DATA_SCRIPT_LEN]) {
    ...
    uint8_t data_len = 0;
    size_t data_start = 0;
    if ((in == NULL) || (out == NULL) || (script_len < expected_len) || (script_len > MAX_DATA_SCRIPT_LEN) || (in->size - in->offset < script_len)) {
...
uint16_t parse_output_script(buffer_t *in, size_t script_len, output_script_info_t *out) {
    ...
    switch(identify_script(in, script_len)) {
      ...
      case SCRIPT_DATA:
          ...
          G_create_token_data.data_len = read_data_script(
              in,
              script_len,
              G_create_token_data.data);
