...
int handler_send_create_token_data(uint8_t token_type, buffer_t *cdata) {
    ...
    if (!(buffer_read_u8(cdata, &G_context.token.version) &&
          ...
          buffer_read_bytes(cdata, G_context.token.name, MAX_TOKEN_NAME_LEN, G_context.token.name_len) &&
          buffer_read_u8(cdata, &G_context.token.symbol_len) &&
          buffer_read_bytes(cdata, G_context.token.symbol, MAX_TOKEN_SYMBOL_LEN, G_context.token.symbol_len))) {
    ...
    if (!is_printable((char *) G_context.token.symbol, (int) G_context.token.symbol_len)) return io_send_sw(SW_UNPRINTABLE_STRING);
    if (!is_printable((char *) G_context.token.name, (int) G_context.token.name_len)) return io_send_sw(SW_UNPRINTABLE_STRING);
    ...
    PRINTF("[+] [send_create_token_data] Token version=%d, namelen=%d, symbollen=%d\n", G_context.token.version, G_context.token.name_len, G_context.token.symbol_len);
